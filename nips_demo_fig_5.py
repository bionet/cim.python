...
Ph_hhat_RMSE = math.sqrt(dt*np.trapz(Ph_hhat_err[t_frame_idx]**2)/    # compute the RMSE
                       (len(t_frame_idx)*dt));
print Ph_hhat_RMSE
print "MMSE time %s"  % repr(time.clock()-t0)

fig = p.figure(num='NIPS 2010 Fig. 5',figsize=(20,15),dpi=300, facecolor='w')
fig.suptitle('NIPS 2010 Figure 5\n' +
             'A.A. Lazar and Y.B. Slutskiy',
             fontsize=24)
# plot the input stimulus u
ax = fig.add_subplot(3,2,1,
                     xlim=(0,1.4),ylim=(-1,1), ylabel='Amplitude', xlabel='Time, [s]',
...
ax = fig.add_subplot(3,2,3,
                     xlim=(0,1.4),ylim=(0,1.2), xlabel='Time, [s]',
                     yticks=[],yticklabels=[],
...
ax.psd(h,pad_to=len(t),Fs=1/dt,sides='twosided',
       NFFT=len(t),
       #noverlap=int(0.05*len(t_proper)/2.),
...
print "Figure time %s"  % repr(time.clock()-t0)
p.savefig('nips_fig_5.png',dpi=300)
